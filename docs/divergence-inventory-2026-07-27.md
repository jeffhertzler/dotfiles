...
| D57 | LazyGit editor bridge | All four profiles use the custom lazygit-nvim bridge rather than LazyGit's built-in nvim preset. Unix invokes it directly. Native Windows LazyGit runs editor commands through cmd.exe, so its rendered config launches the same script explicitly through Git Bash. Its in-Neovim branch uses headless RPC calls so named-pipe query results are not polluted by terminal control sequences; after opening the file it attempts a deferred `q` at 100 ms and retains the proven 250 ms attempt as a no-op fallback, allowing a normal status-0 terminal exit instead of killing the terminal buffer. The complete in-Neovim path passed interactively on Windows, WSL, and Arch, and the separate Herdr-popup return path passed interactively on Arch. LazyVim's `<leader>gG` LazyGit binding remains available alongside Neogit on `<leader>gg`. | Corrected and interactively verified | Keep the behavior shared; retain only the required Windows launcher syntax, guarded in-Neovim terminal return, and Herdr/tmux popup return branches. |
