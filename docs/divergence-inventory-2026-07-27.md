...
| D51 | WSL allowlist | WSL manages Git, gh config, Bat, LazyGit and its Neovim bridge, Herdr core/reordering/popup and its helper, Neovim, OpenCode, Pi, Starship/common shell, Zsh plugins, tunnel, and the checkout-guarded Greenlight/Vimme shell helpers. It ignores the remaining workstation tools; the project helpers are present but inert because their ~/dev checkouts are absent. | Conservative choices plus explicit Bat, Pi, LazyGit, and Herdr decisions | Decide which remaining Mac/Arch tools should join the WSL baseline. |
| D52 | Windows allowlist | Windows manages Git, gh config, Bat, LazyGit and its Git-Bash-launched Neovim bridge, Herdr core/reordering/popup and its helper, Neovim, OpenCode, Pi, Starship/common shell, and Git Bash startup. The popup helper is launched through Git Bash's space-free Windows path because Herdr runs native custom commands through cmd.exe. | Conservative plus explicit Bat, gh, Pi, LazyGit, and Herdr decisions | Decide whether to install/share other native tools. |
