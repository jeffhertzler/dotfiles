...
| D44 | bat and bottom | Mac and Arch manage the same bat config. The inert bottom.toml, which contained only comments and an empty [flags] table, was removed from source and live state. WSL and Windows still ignore bat. | Partially decided and implemented | Review bat availability separately; Bottom no longer has managed configuration. |
