...
| D20 | LazyGit | All four profiles use the same custom lazygit-nvim behavior. Unix renders direct helper commands; native Windows renders the same commands through Git Bash because LazyGit launches them through cmd.exe. WSL and native Windows passed interactive edit-return testing. Two source templates still encode only that launcher difference. | Functionally unified; structural cleanup pending | Test Mac and Arch interactively, then collapse the duplicate templates if their launcher behavior also passes. |
...
| D50 | macOS allowlist | Mac manages Git, gh config, LazyGit, Herdr core and reordering, Neovim, OpenCode, Pi, Starship/common shell, Greenlight/Vimme shell helpers, Atuin, bat, LazyDocker, Posting, Yazi, tunnel, and lazygit-nvim. It intentionally no longer manages legacy tmux/Workmux or inert Bottom configuration. It still ignores agents, SSH, and the remaining local helpers. | Conservative choices plus explicit legacy, Pi, and Herdr decisions | This remains the main list to reconsider if greater unification is desired. |
| D51 | WSL allowlist | WSL manages Git, gh config, Bat, LazyGit and its Neovim bridge, Herdr core and reordering, Neovim, OpenCode, Pi, Starship/common shell, Zsh plugins, tunnel, and the checkout-guarded Greenlight/Vimme shell helpers. It ignores the remaining workstation tools; the project helpers are present but inert because their ~/dev checkouts are absent. | Conservative choices plus explicit Bat, Pi, LazyGit, and Herdr decisions | Decide which remaining Mac/Arch tools should join the WSL baseline. |
| D52 | Windows allowlist | Windows manages Git, gh config, Bat, LazyGit and its Git-Bash-launched Neovim bridge, Herdr core and reordering, Neovim, OpenCode, Pi, Starship/common shell, and Git Bash startup. | Conservative plus explicit Bat, gh, Pi, LazyGit, and Herdr decisions | Decide whether to install/share other native tools. |
...
| D57 | LazyGit editor bridge | All four profiles use the custom lazygit-nvim bridge rather than LazyGit's built-in nvim preset. Unix invokes it directly. Native Windows LazyGit runs editor commands through cmd.exe, so its rendered config launches the same script explicitly through Git Bash. Its bridge uses headless RPC calls so named-pipe query results are not polluted by terminal control sequences; after opening the file it sends LazyGit a deferred `q`, allowing a normal status-0 terminal exit instead of killing the terminal buffer. A path containing spaces and the full native Neovim edit-return cycle passed interactively. LazyVim's `<leader>gG` LazyGit binding remains available alongside Neogit on `<leader>gg`. | Corrected and interactively verified on Windows | Keep the behavior shared; retain only the required Windows launcher syntax and native terminal-return branch. |
