...
| Yazi | Absent | 26.5.6 official binaries | 26.5.6 via Homebrew | 26.5.6 via pacman |
...
| D42 | Yazi | WSL, Mac, and Arch manage Yazi. Mac renders open; WSL and Arch render xdg-open. WSL uses Yazi's official Linux binaries because Ubuntu has no native package; native Windows/Git Bash remains intentionally out of scope. | Decided and implemented | Keep Yazi on the three Unix profiles and retain the required opener split. |
