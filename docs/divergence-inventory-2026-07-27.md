...
| Yazi | 26.5.6 via WinGet | 26.5.6 official binaries | 26.5.6 via Homebrew | 26.5.6 via pacman |
| jq, zoxide | jq 1.8.2 and zoxide 0.10.0 via WinGet | jq 1.8.2 and zoxide 0.10.0 official binaries | Both via Homebrew | Both via pacman |
...
| D27 | Herdr advanced commands | Direct Ctrl-H/J/K/L navigation, local left/up directional splits, tab/workspace reordering, and the Bash/fzf popup launcher are shared by Windows, WSL, Mac, and Arch. The shared navigation adapter forwards into Neovim when appropriate and otherwise focuses the neighboring Herdr pane; this is separate from Herdr's navigate-mode-only `navigate_pane_*` settings. Reordering uses a per-user runtime/temp lock keyed to the Herdr socket, including native Windows named pipes. The low-value session picker and its `hs` alias were retired. Arch alone retains plugin-dependent resizing/full-layout splits, Worktrunk actions, and the popup's legacy Workmux entry. | Decided and implemented | Keep the portable behavior shared and the plugin/legacy additions Arch-only. |
| D28 | Herdr plugin subtree | Herdr plugin actions and plugin configuration are Arch-only. The installed plugins are user-shared under ~/.config/herdr/plugins and remain outside Chezmoi ownership. Portable navigation no longer depends on that subtree because the upstream herdr-splits manifest excludes native Windows. | Conservative | Decide later whether Mac should share the remaining plugin set. |
...
| D42 | Yazi | All four profiles manage one Yazi config. Mac renders open, WSL/Arch render xdg-open, and native Windows renders start plus a Git-Bash-launched Neovim opener. Windows uses WinGet and YAZI_CONFIG_HOME to retain the XDG path; WSL uses official Linux binaries because Ubuntu has no native package. External Yazi uses the shared yazi-nvim bridge, while yazi.nvim retains its independent in-Neovim DDS integration. Ordinary terminal copy/paste and Yazi's internal file yank/paste need no Linux clipboard package. No Wayland/X11 clipboard provider is installed for SSH-only Arch or WSL; a Windows-native or OSC 52 path can be added later only if copying Yazi paths to the local OS clipboard becomes useful. | Decided and implemented | Keep Yazi shared on all four profiles with only OS launcher syntax conditionalized. |
...
| D49 | Local helper scripts | agent-review, lazygit-nvim, yazi-nvim, nvim-target.sh, herdr-navigate.mjs, and herdr-popup-picker are shared by Windows, WSL, Mac, and Arch. Agent Review uses the shared exact-target resolver before its unique-live-server fallback; all four machines use Bash 5.3 and jq, while newline stripping makes base64 encoding compatible with both GNU/uutils and macOS FreeBSD base64. LazyGit and external Yazi source the same resolver but retain thin app-specific adapters. Herdr navigation uses one Node adapter so native Windows and Unix receive the same Neovim-aware behavior without requiring the Arch plugin subtree. The popup retains its Bash/fzf UI and enables Workmux only from the Arch binding. tunnel remains profile-specific on WSL/Mac; tmux helpers and worktrunk-seed remain Arch-only legacy. | Decided and implemented | Keep the portable helpers shared and the explicitly legacy/profile-specific helpers scoped. |
...
| D60 | Neovim registry portability | The original AgentBridge-owned registry fallback used Vim's unavailable `getuid()` function, allowed `:` from Herdr pane IDs in filenames, and assumed Unix sockets when LazyGit validated a published server. Publication now belongs to the general config.nvim_server module because LazyGit and Yazi consume it independently of AgentBridge. Windows publishes under its temp directory, Unix uses XDG_RUNTIME_DIR or libuv's user id, pane IDs use a portable filename, and the shared nvim-target.sh resolver validates servers through headless Neovim RPC so native Windows named pipes work. Native Windows direct-NVIM and Herdr-registry Yazi routes passed end-to-end named-pipe tests; prior Unix registry and AgentReview tests remain applicable. | Corrected and implemented | Keep one publisher and one exact-target resolver with only the runtime-root selection conditionalized by platform. |
...
| yazi-nvim | Cross-platform bridge from external Yazi back to the originating Neovim instance | Managed on all four machines. It shares target resolution with LazyGit, opens the selection, and asks Yazi to quit; native yazi.nvim remains independent. |
| nvim-target.sh | Shared exact-target resolver for external TUI adapters | Managed on all four machines under ~/.local/lib. It resolves a live `$NVIM`, then the active Herdr pane registry, then the legacy tmux window target. |
...
- Yazi 26.5.6, jq 1.8.2, and zoxide 0.10.0 are installed through WinGet packages sxyazi.yazi, jqlang.jq, and ajeetdsouza.zoxide. YAZI_CONFIG_HOME points to ~/.config/yazi and YAZI_FILE_ONE points to Git for Windows' file.exe; remove those two user variables when uninstalling Yazi if they are no longer useful.
...
- Yazi 26.5.6 was installed from the official x86_64 GNU/Linux release as ~/.local/bin/yazi and ~/.local/bin/ya. Removing those two binaries uninstalls it; its configuration is managed separately by Chezmoi.
- jq 1.8.2 and zoxide 0.10.0 were installed from their checksummed official x86_64 Linux release artifacts under ~/.local/bin. No Wayland or X11 clipboard package was added.
...
After the pane-navigation interruption is resolved, return to this deferred sequence: decide the remaining Mac Herdr plugins, decide Worktrunk outside Arch, revisit Mise/Volta, then merge `normalize/multi-platform` into `master`.
