...
assert(review.add == nil and review.annotation and review.session and review.status and review.ui)
...
local counts = review.status.counts()
assert(counts.current == 1 and counts.total == 1 and counts.stale == 0)
assert(review.status.text() == "Review 1/1" and review.status.has())
...
assert(review.status.text() == "" and not review.status.has())
