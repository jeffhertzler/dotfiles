...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/native-review-tests.XXXXXX")
trap 'rm -rf "$tmp"' EXIT
...
run_case() {
  ...
  NATIVE_REVIEW_PLUGIN_ROOT="$plugin_root" \
  NATIVE_REVIEW_TEST_CASE="$tests_root/cases/$name.lua" \
  NATIVE_REVIEW_TEST_FILE="$file" \
  NATIVE_REVIEW_TEST_TMP="$tmp/$name-work" \
  NVIM_NATIVE_REVIEW_STATE="$state" \
...
for name in core columns rpc bridge; do
  file="$tmp/$name.lua"
  state="$tmp/$name.json"
  make_file "$file"
...
printf '\nAll native-review tests passed.\n'
